...
- [x] 在 **Settings > Branches** 或 Rulesets 为默认分支启用 branch protection：要求 PR、CI 通过和禁止强推。
- [x] 确认 SECURITY.md 的私密报告路径实际可用，不发布虚构邮箱。

## 版本质量

- [x] `gofmt`、`go vet ./...`、`go test ./...`、`go test -race ./...`、`govulncheck ./...` 全部通过。
- [ ] README 最小示例可编译并生成预期 JSONL；Gin、net/http、Metric、OTLP 示例可运行。
- [x] 清理本机路径、内部决策术语、失效 badge、临时日志和敏感数据。
