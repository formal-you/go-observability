...
- 注：ErrorType 文法与命名空间部分已被 [ADR-0016](../errors/0016-errortype-otel-grpc-standard-enum.md) 取代；ErrorCode 的 SCOPE.OPERATION.REASON 部分保持有效。
...
- ErrorType 复用 OTel/gRPC 标准枚举（gRPC canonical code，单段 UPPER_SNAKE，闭合枚举），文法见 [ADR-0016](../errors/0016-errortype-otel-grpc-standard-enum.md)；`ParseErrorType` 校验枚举成员。
