...
> 真源分工：本文是「一个开发任务从需求下达到提交」的执行剧本；Issue/PR/合并与授权语义以 [GitOps SOP](gitops-sop.md) 为准；术语以 [CONTEXT.md](../CONTEXT.md) 为准；黑盒验收契约以 [testing.md](testing.md) 为准。
...
| 角色 | 定位 | 职责 | 不做 |
|---|---|---|---|
| masterAgent（主 Agent） | 项目经理 + 产品经理 + 架构师 | 需求收敛、定契约与验收、拆票排期、给 subAgent 派任务卡、整合成果、守门禁、向用户汇报与确认 | 不直接写核心实现（小改动除外）；不越过 [GitOps SOP](gitops-sop.md) 授权语义做 push/merge |
...
|---|---|---|---|
| 0 需求澄清与收敛 | masterAgent | `triage` / `to-questionnaire` / `grill-with-docs` / `wait-what` | 契约草案（问题/范围非范围/验收/影响）、假设清单、需用户确认项；写入 `docs/issues/NNNN-<slug>.md` |
...
| 7 收尾提交 | masterAgent | `handoff`（跨会话时） | 更新 issue 状态；显式路径暂存；Conventional Commit；按 [GitOps SOP](gitops-sop.md) 授权语义 push/PR/merge |
...
| [gitops-sop.md](gitops-sop.md) | Issue/PR/CI/合并/归档的 GitOps 治理与授权语义 | 具体怎么开发一个任务 |