...
const (
	...
	// 用户（semconv user）与业务 vendor 命名空间
	KeyUserID Key = "user.id"
	// KeyAppUserID 已弃用：新事件输出 KeyUserID（user.id）。
	KeyAppUserID   Key = "app.user_id"
	KeyAppTenantID Key = "app.tenant_id"
	KeyAppResult   Key = "app.result"
	// KeyAppBusinessCode 已弃用：新事件统一使用 KeyErrorCode。
	...
	// KeyAppOperation 已弃用：错误码统一使用 KeyErrorCode。
	...
	// KeyErrorCode 稳定具体错误码（error.code：error.type 是 OTel 语义，error.code 是企业级扩展）。
	KeyErrorCode Key = "error.code"
	// KeyAppErrorCode 已弃用：新事件统一使用 KeyErrorCode。
	KeyAppErrorCode           Key = KeyErrorCode
	...
	KeyAppStacktraceTruncated Key = "app.stacktrace_truncated"
