...
**先定位任务类型（三组入口）**：
- 开发任务执行（需求收敛 → 验收 → 提交，含多 Agent 编排）：[开发流程体系](docs/dev-sop/README.md)
- GitOps 治理（分支 / Issue / PR / 发布 / 合并）：[GitOps 治理](docs/gitops/README.md)
...
5. **完成即提交**：按 [GitOps SOP](docs/gitops/gitops-sop.md) 完成分级门禁验证后，用显式路径暂存并创建 Conventional Commit；不 push，除非用户明确要求。
