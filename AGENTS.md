...
- **任何代码或文档变更**：先读 [GitOps SOP](docs/gitops-sop.md)，其中包含验证、文档同步和 Git 提交门禁。
...
- **重大架构或公共 API 变更（新增/更新 ADR）**：先读 [ADR 约定](docs/adr/README.md) 与 [GitOps SOP](docs/gitops-sop.md) 的变更步骤。
...
- **开始一个开发任务（含多 Agent 编排）**：先读 [开发任务 SOP](docs/dev-sop.md)；masterAgent=项目经理/产品经理/架构师，subAgent=探索/开发核心；提交与 PR 环节仍以 [GitOps SOP](docs/gitops-sop.md) 为准。
...
2. **核心零依赖**：`log/` 只依赖标准库；允许的依赖方向与 OTel 边界以 [架构说明](docs/architecture.md) 为准。
...
5. **完成即提交**：按 [GitOps SOP](docs/gitops-sop.md) 完成全量门禁后，用显式路径暂存并创建 Conventional Commit；不 push，除非用户明确要求。
