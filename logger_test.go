...
func TestLoggerEmitWritesNormalizedAttrs(t *testing.T) {
	...
	for _, k := range []string{"trace_id", "span_id", "event.name", "http.request.method", "mall.result"} {
		if _, ok := attrs[k]; !ok {
...
func TestLoggerBaseMetadataFillsMissing(t *testing.T) {
	...
	l.Emit(context.Background(), BusinessEvent{
		Data: BusinessPayload{EventName: "business.order.paid", Result: ResultSuccess},
	})
	attrs := attrMap(w.attrsList[0])
	if attrs["level"] == nil {
